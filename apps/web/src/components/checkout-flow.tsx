      ...
      <Dialog.Portal>
        ...
        <Dialog.Content dir={rtl ? 'rtl' : 'ltr'} className="savoraille-dialog-surface fixed inset-x-0 bottom-0 z-[110] max-h-[94svh] overflow-y-auto overflow-x-hidden rounded-t-3xl border border-[#C6A15B]/35 bg-[#FAF6EC] text-[#241F19] shadow-[0_-20px_70px_rgba(3,16,31,0.35)] outline-none sm:inset-x-6 sm:bottom-auto sm:left-1/2 sm:top-1/2 sm:w-[min(960px,calc(100vw-48px))] sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-3xl rtl:sm:left-auto rtl:sm:right-1/2 rtl:sm:translate-x-1/2">
          ...
          {step === 'service' ? (
            ...
            <motion.form key={`details-${service}`} variants={stepVariants} initial="enter" animate="center" exit="exit" transition={stepTransition} onSubmit={handleSubmit(submit)} noValidate className="grid min-w-0 lg:grid-cols-[1fr_300px]">
              <div className="min-w-0 p-5 sm:p-8">
                ...
                <div className="mt-6 grid min-w-0 gap-4 sm:grid-cols-2">
                  ...
                  {service === 'dine-in' ? <label className="min-w-0 text-sm font-bold text-[#1E3A5F] sm:col-span-2"><span className="flex items-center gap-2"><UsersRound className="size-4 text-[#C6A15B]" />{tr('Nombre de personnes')}</span><Controller name="guests" control={control} render={({ field }) => <FormSelect value={field.value ?? '2'} onChange={field.onChange} options={guestOptions} label={tr('Nombre de personnes')} rtl={rtl} />} />{error('guests')}</label> : null}
                  {service === 'delivery' ? <label className="min-w-0 text-sm font-bold text-[#1E3A5F] sm:col-span-2"><span className="flex items-center gap-2"><MapPin className="size-4 text-[#C6A15B]" />{tr('Adresse de livraison')}</span><input {...register('address')} autoComplete="street-address" placeholder={tr('Rue, numéro, ville et code postal')} aria-invalid={!!errors.address} className={fieldClass} />{error('address')}</label> : null}
                  <label className="min-w-0 text-sm font-bold text-[#1E3A5F]"><span className="flex items-center gap-2"><UserRound className="size-4 text-[#C6A15B]" />{tr('Votre nom')}</span><input {...register('name')} autoComplete="name" placeholder={tr('Nom et prénom')} aria-invalid={!!errors.name} className={fieldClass} />{error('name')}</label>
                  <label className="min-w-0 text-sm font-bold text-[#1E3A5F]"><span className="flex items-center gap-2"><Phone className="size-4 text-[#C6A15B]" />{tr('Téléphone')}</span><input {...register('phone')} type="tel" autoComplete="tel" placeholder="+33 6 00 00 00 00" aria-invalid={!!errors.phone} className={fieldClass} />{error('phone')}</label>
                  <label className="min-w-0 text-sm font-bold text-[#1E3A5F] sm:col-span-2">{tr('Allergies ou demande particulière')}<textarea {...register('notes')} rows={3} placeholder={tr('Facultatif')} className="mt-2 w-full min-w-0 max-w-full resize-none rounded-lg border border-[#1E3A5F]/14 bg-white px-4 py-3 text-sm outline-none focus:border-[#C6A15B] focus:ring-2 focus:ring-[#C6A15B]/35" /></label>
                </div>
              </div>
