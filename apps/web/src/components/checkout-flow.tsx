...
export function CheckoutFlow({ itemCount, total }: { itemCount: number; total: number }) {
  ...
  const chooseService = (mode: ServiceMode) => { setTransitionDirection(1); setService(mode); reset({ date: today, time: mode === 'takeaway' ? 'asap' : '20:00', guests: '2', name: '', phone: '', address: '', notes: '' }); setStep('details'); };
  const submit = () => { setTransitionDirection(1); setReference(`SV-${Date.now().toString().slice(-5)}`); setStep('done'); };
  ...
  const travelDirection = transitionDirection * (rtl ? -1 : 1);
  const stepVariants = {
    enter: (direction: number) => reduceMotion ? { opacity: 0 } : { opacity: 0, x: 24 * direction },
    center: { opacity: 1, x: 0 },
    exit: (direction: number) => reduceMotion ? { opacity: 0 } : { opacity: 0, x: -18 * direction },
  };
    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content dir={rtl ? 'rtl' : 'ltr'} className="savoraille-dialog-surface fixed inset-x-0 bottom-0 z-[110] max-h-[94svh] overflow-y-auto overflow-x-hidden rounded-t-3xl border border-[#C6A15B]/35 bg-[#FAF6EC] text-[#241F19] shadow-[0_-20px_70px_rgba(3,16,31,0.35)] outline-none sm:inset-x-6 sm:bottom-auto sm:left-1/2 sm:top-1/2 sm:w-[min(960px,calc(100vw-48px))] sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-3xl rtl:sm:left-auto rtl:sm:right-1/2 rtl:sm:translate-x-1/2">
          ...
          {step === 'service' ? (
            <motion.div key="service" custom={travelDirection} variants={stepVariants} initial="enter" animate="center" exit="exit" transition={stepTransition} className="p-5 sm:p-8">
            ...
            <motion.form key={`details-${service}`} custom={travelDirection} variants={stepVariants} initial="enter" animate="center" exit="exit" transition={stepTransition} onSubmit={handleSubmit(submit)} noValidate className="grid lg:grid-cols-[1fr_300px]">
