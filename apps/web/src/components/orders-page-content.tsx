      ...
      <div className="mx-auto max-w-[1200px]">
        ...
        {lines.length === 0 ? (
          <section className="mx-auto grid max-w-xl place-items-center py-20 text-center">
            ...
            <LocalizedLink href="/carte" className="mt-7 inline-flex items-center gap-2 rounded-lg bg-[#1E3A5F] px-6 py-4 font-bold text-[#FAF6EC]">{tr('Découvrir la carte')}<ArrowRight className="size-4 rtl:-scale-x-100" /></LocalizedLink>
          </section>
        ) : (
          <>
            ...
            <section className="grid gap-4" aria-label={tr('Articles de votre commande')}>
              {detailedLines.map(({ item, quantity, menuId, customization }) => {
                ...
                return <article key={`${menuId}-${item.id}`} className="grid grid-cols-[88px_1fr] gap-4 rounded-2xl border border-[#1E3A5F]/12 bg-white p-3 shadow-[0_8px_24px_rgba(30,58,95,0.08)] sm:grid-cols-[128px_1fr_auto] sm:items-center sm:gap-5 sm:p-4">
                  <LocalizedLink href={`/commander/${item.id}`} className="relative aspect-square overflow-hidden rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-[#C6A15B]"><AppImage src={item.image} alt={tr(item.name)} fill sizes="128px" className="object-cover" /></LocalizedLink>
                  <div className="min-w-0"><LocalizedLink href={`/commander/${item.id}`} className="font-display text-xl font-semibold text-[#1E3A5F] outline-none hover:text-[#7C2438] focus-visible:ring-2 focus-visible:ring-[#C6A15B] sm:text-2xl">{tr(item.name)}</LocalizedLink><p className="mt-1 text-sm font-bold text-[#7C2438]">{item.price}{customization?.unitSupplement ? ` · +${money(customization.unitSupplement)}` : ''}</p>{customization?.optionLabels.length ? <div className="mt-2 flex flex-wrap gap-1.5">{customization.optionLabels.map((label) => <span key={label} className="rounded-full border border-[#C6A15B]/35 bg-[#FAF6EC] px-2.5 py-1 text-[0.68rem] font-bold text-[#1E3A5F]">+ {tr(label)}</span>)}</div> : null}<button type="button" onClick={() => removeItem(menuId, item.id)} className="mt-3 inline-flex items-center gap-1.5 text-xs font-bold text-[#241F19]/48 outline-none hover:text-[#7C2438] focus-visible:ring-2 focus-visible:ring-[#C6A15B]"><Trash2 className="size-3.5" />{tr('Retirer')}</button></div>
              ...
              <LocalizedLink href="/carte" className="mt-2 inline-flex w-fit items-center gap-2 text-sm font-bold text-[#7C2438] outline-none focus-visible:ring-2 focus-visible:ring-[#C6A15B]"><Plus className="size-4" />{tr('Ajouter un autre plat')}</LocalizedLink>
            </section>
